...
namespace UGF.EditorTools.Psd2UGUI
{
    ...
    {
        public PsdLayerNode LayerNode => GetComponent<PsdLayerNode>();
        ...
        {
            if (!LayerNode.IsMainUIType || LayerNode.UIType == GUIType.Null) return null;

            if (uiInstance == null)
            {
                var rule = UGUIParser.Instance.GetRule(LayerNode.UIType);
                ...
                uiInstance = Instantiate(rule.UIPrefab, Vector3.zero, Quaternion.identity);
                if (LayerNode.IsMainUIType)
                {
                    uiInstance.name = LayerNode.GetName();
                    var key = uiInstance.GetComponent<UIStringKey>() ?? uiInstance.AddComponent<UIStringKey>();
                    key.Key = gameObject.GetInstanceID().ToString();
                }
            }
